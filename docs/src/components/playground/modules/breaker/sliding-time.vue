<script lang="ts" setup>
  ...
  const cbWindowSize = ref<number>(30000);
  ...
  // Constants
  const slidingTime = new Mollitia.SlidingTimeBreaker({
  ...
  // Handlers
  watch([cbWindowSize], () => {
    slidingTime.slidingWindowSize = cbWindowSize.value;
  });
  watch([cbMinimumNumberOfCalls], () => {
    slidingTime.minimumNumberOfCalls = cbMinimumNumberOfCalls.value;
  });
  watch([cbFailureRateThreshold], () => {
    slidingTime.failureRateThreshold = cbFailureRateThreshold.value;
  });
  watch([cbSlowCallDurationThreshold], () => {
    slidingTime.slowCallDurationThreshold = cbSlowCallDurationThreshold.value;
  });
  watch([cbSlowCallRateThreshold], () => {
    slidingTime.slowCallRateThreshold = cbSlowCallRateThreshold.value;
  });
  watch([cbPermittedNumberOfCallsInHalfOpenState], () => {
    slidingTime.permittedNumberOfCallsInHalfOpenState = cbPermittedNumberOfCallsInHalfOpenState.value;
  });
  watch([cbOpenStateDelay], () => {
    slidingTime.openStateDelay = cbOpenStateDelay.value;
  });
  watch([cbHalfOpenStateMaxDelay], () => {
    slidingTime.halfOpenStateMaxDelay = cbHalfOpenStateMaxDelay.value;
  });
  ...
  // Lifecycle
  onMounted(() => {
    slidingTime.on('state-changed', handleStateChange);
  });
  onUnmounted(() => {
    slidingTime.off('state-changed', handleStateChange);
  });
</script>

<template>
  <div class="sliding-time">
